...
class secure_error_handler(Middleware):
    async def on_call_tool(self, context: MiddlewareContext, call_next):
        """
        Catches exceptions from all subsequent middleware and tools.
        Returns the error in a ToolResult object with structured content.
        This is done to prevent crashing when LLM sends invalid input to a tool or creates unhandled exceptions.
        """
        try:
            return await call_next(context)
        except Exception as e:
            return ToolResult(structured_content={"result": "Error in tool execution: " + str(e)})
...
class TaskStorageMcpServer:
    def __init__(self, clean_db=False):
        ...
        self.mcp.add_middleware(secure_error_handler())
